...
pub fn TripDetail(id: i64) -> Element {
    let trip = use_resource(move || api::trip(id));
    let photos = use_resource(move || api::photos(id));
    let track = use_resource(move || api::track(id));
    ...
    rsx! {
        ...
        match &*photos.read_unchecked() {
            ...
            Some(Ok(photos)) => rsx! { Gallery { photos: photos.clone() } },
        }
    }
}
...
#[component]
fn Gallery(photos: Vec<PhotoView>) -> Element {
    rsx! {
