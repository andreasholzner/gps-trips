...
/// Everything the two screens can narrow the list by. Every text field is
/// kept as the raw string the input holds: blank means "don't filter on this
/// dimension", exactly as the server's own parser reads it, so an
/// half-typed value never needs a separate "is it valid yet" state here.
