...
| **US-7** | As the owner, a **trip detail page** lets me relive a trip. | Shows the track on an OSM map, an elevation profile, and a photo gallery with map markers. |
| **US-8** | As the owner, trip **stats are computed automatically**, never entered by hand. | Distance, ascent, descent, duration, bounding box, and point count are derived from the GPX at import. |
